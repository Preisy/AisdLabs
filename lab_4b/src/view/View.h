...
public:
    [[noreturn]] void run() { // todo делегировать saw к view
        ...
        while (true) {
            ...
            for (int i = 0; i < arr.getLength(); ++i) {
                ...
                if (arr[i][0] == "ploh") {
                    ...
                    MapController::ActionResult<std::string> response = mapController.find(arr[i][1]);
                    responseType = response.getStatus();
                    if (responseType == MapController::ResponseType::Ok) {
                        std::cout << *response << std::endl;
                    }
                } else if (arr[i][0] == "sfind") {
                    MapController::ActionResult<std::string> response = mapController.specialFind(arr[i][1]);
                    responseType = response.getStatus();
                    if (responseType == MapController::ResponseType::Ok) {
                        std::cout << *response << std::endl;
                    }
                    ...
                    if (arr[i][1] != "") {
                        responseType = MapController::ResponseType::commandError;
                    } else {
                        if (autoWriting) {
...
//        std::cout << ">>> ";
    ...
    void help() {
        ...
        std::cout << "8. mock;" << std::endl;
        std::cout << "9. saw; - switch auto writing" << std::endl;
        std::cout << std::endl;
        std::cout << "You can try to enter an example command:" << std::endl;
